...
const getWorkplaces = async (req, res) => {
  ...
  try {
    const workplacesInstances = await workplace.findAll({
      where: { unionId },
      include: {
        model: chat,
        as: 'chats'
    ...
    const workplaces = []
    for (const wp of workplacesInstances) {
      workplaces.push(wp.dataValues)
    }
    if (workplaces.length === 0) {
      return res.status(404).json({ status: 'fail', message: 'No workplaces found for this union ID.' });
    }

    res.status(200).json({ status: 'success', workplaces });
  } catch (error) {
